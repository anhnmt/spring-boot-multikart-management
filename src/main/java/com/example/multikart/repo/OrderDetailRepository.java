...
public interface OrderDetailRepository extends CrudRepository<OrderDetail, Long> {
    @Query("SELECT new com.example.multikart.domain.dto.OrderDetailDTO(od, p, pi)\n" +
            ...
            "LEFT JOIN ProductImage pi on p.productId = pi.productId and pi.position = (Select min(position) from ProductImage where productId = p.productId) AND pi.status = :status\n" +
            "WHERE o.status <> :orderStatus\n" +
            " AND p.status = :status\n" +
            " AND o.orderId = :orderId\n" +
            " AND od.orderId = :orderId")
    List<OrderDetailDTO> findAllByOrderIdAndStatusNot(Long orderId, Integer orderStatus, Integer status);
}
